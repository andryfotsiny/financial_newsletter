...
export function Navigation({ className, onLinkClick }: NavigationProps) {
  ...
  return (
    <nav
      className={cn(
        "flex items-end space-x-6 lg:space-x-8 text-sm font-medium",
        className
      )}
    >
      {navItems.map((item) => (
        <Link
          ...
          onClick={onLinkClick}
          className={cn(
            "relative flex justify-center transition-colors hover:text-foreground pb-3 group whitespace-nowrap min-w-20",
